...
public class OKBlock extends Message implements BlockInf{
    ...
    private static final Logger log = LoggerFactory.getLogger(OKBlock.class);

    /** How many bytes are required to represent a block header WITHOUT the trailing 00 length byte. */
//    public static final int HEADER_SIZE = 80;
    /** OKBlockHeader大小为104Bytes. */
...
//    public static final int MAX_BLOCK_SIZE = 1 * 1000 * 1000;
    public static final int MAX_BLOCK_SIZE = 8 * 1000 * 1000;	//8M
...
//    private long nonce;
    ...
    @Nullable List<OKTransaction> transactions;
    ...
    OKBlock(NetworkParameters params, int setVersion) {
        super(params);
...
//        difficultyTarget = 0x1d07fff8L;
     ...
     * @throws ProtocolException
     */
    public OKBlock(NetworkParameters params, byte[] payloadBytes, int offset, @Nullable Message parent, MessageSerializer serializer, int length)
            throws ProtocolException {
    ...
    protected void parseTransactions(final int transactionsOffset) throws ProtocolException {
        ...
        transactions = new ArrayList<OKTransaction>(numTransactions);
        for (int i = 0; i < numTransactions; i++) {
            OKTransaction tx = new OKTransaction(params, payload, cursor, this, serializer, UNKNOWN_LENGTH);
...
//    }
    ...
    public OKBlock cloneAsHeader() {
        OKBlock block = new OKBlock(params, BLOCK_VERSION_GENESIS);
        copyBitcoinHeaderTo(block);
        return block;
    }

    /** Copy the block without transactions into the provided empty block. */
    protected final void copyBitcoinHeaderTo(final OKBlock block) {
...
//    }
    ...
    private void checkTransactions(final int height, final EnumSet<VerifyFlag> flags)
        ...
        if (flags.contains(OKBlock.VerifyFlag.HEIGHT_IN_COINBASE) && height >= BLOCK_HEIGHT_GENESIS) {
...
//        checkProofOfWork(true);
    ...
    public boolean equals(Object o) {
        ...
        return getHash().equals(((OKBlock)o).getHash());
    }
    ...
    public void addTransaction(OKTransaction t) {
        addTransaction(t, true);
    }

    /** Adds a transaction to this block, with or without checking the sanity of doing so */
    void addTransaction(OKTransaction t, boolean runSanityChecks) {
        unCacheTransactions();
        if (transactions == null) {
            transactions = new ArrayList<OKTransaction>();
        }
        t.setParent(this);
...
//        this.hash = null;
    ...
    @Nullable
    public List<OKTransaction> getTransactions() {
    ...
    void addCoinbaseTransaction(byte[] pubKeyTo, Coin value, final int height) {
        unCacheTransactions();
        transactions = new ArrayList<OKTransaction>();
        OKTransaction coinbase = new OKTransaction(params);
        final ScriptBuilder inputBuilder = new ScriptBuilder();

        if (height >= OKBlock.BLOCK_HEIGHT_GENESIS) {
     ...
     */
    @VisibleForTesting
    public OKBlock createNextBlock(Address to, int version, long time, int blockHeight) {
...
//        b.solve();
    ...
    @VisibleForTesting
    public OKBlock createNextBlock(@Nullable Address to, TransactionOutPoint prevOut) {
    ...
    @VisibleForTesting
    public OKBlock createNextBlock(@Nullable Address to, Coin value) {
        return createNextBlock(to, BLOCK_VERSION_GENESIS, null, getTimeSeconds() + 5, pubkeyForTesting, value, BLOCK_HEIGHT_UNKNOWN);
    }

    @VisibleForTesting
    public OKBlock createNextBlock(@Nullable Address to) {
        return createNextBlock(to, FIFTY_COINS);
    }

    @VisibleForTesting
    public OKBlock createNextBlockWithCoinbase(int version, byte[] pubKey, Coin coinbaseValue, final int height) {
        return createNextBlock(null, version, (TransactionOutPoint) null,
                               (int)Utils.currentTimeSeconds(), pubKey, coinbaseValue, height);
    }
     ...
     */
    @VisibleForTesting
    OKBlock createNextBlockWithCoinbase(int version, byte[] pubKey, final int height) {
        return createNextBlock(null, version, (TransactionOutPoint) null,
                               (int)Utils.currentTimeSeconds(), pubKey, FIFTY_COINS, height);
    }
